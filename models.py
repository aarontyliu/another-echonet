...
from utils import replace_activation_in_model
...
class EchoNet2(nn.Module):
    def __init__(self, device, encoder_hidden_dim=256):
        super(EchoNet2, self).__init__()
        self.encoder_hidden_dim = encoder_hidden_dim
        # ====================================
        # Semantic segmentation network: UNet
        # ====================================
        self.device = device
        self.unet = ResUNet(n_channels=1, n_classes=1)
        ...
        self.encoder = torch.nn.ModuleList(list(resnet50.children())[:-1])
        self.encoder.append(nn.Flatten())
        self.encoder = torch.nn.Sequential(*self.encoder)

        self.regressor = torch.nn.Sequential(
            *[
                nn.Linear(resnet50.fc.in_features, self.encoder_hidden_dim),
                nn.BatchNorm1d(self.encoder_hidden_dim, momentum=0.01),
        ...
        # ==================
        self.decoder = LSTMDecoder(
            input_dim=resnet50.fc.in_features,
            hidden_dim=256,
            num_layers=2,
            num_outputs=1,
            bidirectional=True,
